...
// ==========================================
// 3. CÁC TRANG QUẢN LÝ (MENU, TABLES, INVENTORY)
// ==========================================
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            <div class="flex items-center gap-4 mb-2">
                                <img id="productPreview" src="${FALLBACK_IMG}" alt="Preview" class="w-20 h-20 rounded-lg object-cover border border-gray-200 bg-gray-50">
                                <div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-semibold mb-1 text-gray-800">Giá <span class="text-red-500">*</span></label>
                                ...
                                <div>
                                    ...
                                    <select id="productCategory" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-red-400 outline-none">
                        ...
                        <div class="bg-gray-50 border border-gray-200 rounded-xl p-4">
                            <div class="flex justify-between items-center mb-3">
                                <label class="block text-sm font-semibold text-gray-800 m-0">Định lượng Nguyên liệu</label>
                                <button type="button" id="btnAddRecipeRow" class="text-xs bg-blue-100 text-blue-600 px-2 py-1 rounded font-bold hover:bg-blue-200">
                                    + Thêm nguyên liệu
                                </button>
                            </div>
                            <p class="text-[11px] text-gray-500 mb-3 leading-relaxed">Chọn nguyên liệu để tự động trừ kho khi bán.</p>
                            <div id="recipeContainer" class="space-y-2 max-h-[220px] overflow-y-auto pr-1"></div>
                        </div>
                    </div>

                    <div class="grid grid-cols-2 gap-4 mt-8 pt-4 border-t border-gray-100">
                        <button type="button" id="btnCancelProductModal" class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold py-3 rounded-lg">Hủy</button>
                        <button type="submit" id="btnSaveProduct" class="bg-red-500 hover:bg-red-600 text-white font-semibold py-3 rounded-lg shadow-sm">Lưu món & Công thức</button>
                    </div>
                </form>
            </div>
        </div>
...
function getTableManagementContent(tables = []) {
        ...
        <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
            ...
            <td class="py-4 px-6">
                <div class="flex gap-2">
                    ...
                    <button class="btn-toggle-status bg-[#4285F4] text-white p-2 rounded-lg hover:bg-blue-600 shadow-sm" data-table="${table.id}" title="Chuyển trạng thái">
                    ...
                    <button class="btn-delete-table bg-[#EA4335] text-white p-2 rounded-lg hover:bg-red-600 shadow-sm" data-table="${table.id}" title="Xóa">
                        ${managerIcons.trash}
                    </button>
                </div>
            </td>
    ...
    return `
        <div class="max-w-5xl mx-auto">
            <div class="flex justify-between items-center mb-8">
                ...
                <button id="btnOpenAddTable" class="bg-[#2ECA6A] hover:bg-green-600 text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-sm">
        ...
        <div id="addTableModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center hidden opacity-0 transition-opacity duration-300">
            <div class="bg-white rounded-3xl w-full max-w-sm p-6 mx-4 transform scale-95 transition-transform duration-300 shadow-xl" id="addTableContent">
                ...
                <form id="addTableForm">
                    <div class="mb-8">
                        <label class="block text-[11px] font-bold text-gray-900 mb-2">Số bàn (Để trống tự động tăng)</label>
                        <input type="number" id="newTableNumber" class="w-full border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-red-400 outline-none text-sm font-medium">
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <button type="submit" class="bg-[#EA4335] hover:bg-red-600 text-white font-bold py-3 rounded-xl">Thêm</button>
        ...
        <div id="qrModal" class="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center hidden opacity-0 transition-opacity duration-300">
            <div class="bg-white rounded-3xl p-8 transform scale-95 transition-transform duration-300 flex flex-col items-center shadow-2xl relative" id="qrContent">
                ...
                <p class="text-xs text-gray-500 mb-6 text-center max-w-[250px]">Khách quét mã này để gọi món trực tiếp vào máy tính.</p>
            </div>
        </div>
    `;
}
...
function getManagerInventoryContent(materialsData = []) {
    const tableRows = materialsData.map(item => `
        <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
            ...
            <td class="py-4 px-4 text-center"><span class="text-base font-extrabold ${item.isLowStock ? 'text-red-600' : 'text-gray-900'}">${item.stock || item.stockQuantity || 0}</span><span class="text-xs text-gray-500"> ${item.unit}</span></td>
            <td class="py-4 px-4 text-center"><span class="px-3 py-1 rounded-full text-xs font-bold bg-green-50 text-green-600 border border-green-100">Đang theo dõi</span></td>
        </tr>
    `).join('');
    return `
        <div class="max-w-5xl mx-auto">
...
// ==========================================
// 4. MỚI: TRANG ĐƠN HÀNG VÀ DOANH THU ĐƯỢC THIẾT KẾ ĐẦY ĐỦ
// ==========================================

function getManagerOrdersContent() {
    return `
        <div class="max-w-6xl mx-auto">
            <h1 class="text-2xl md:text-3xl font-extrabold text-gray-900 mb-8">Quản lý toàn bộ đơn hàng</h1>
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50 text-[11px] text-gray-500 font-bold uppercase tracking-wider border-b border-gray-100">
                    ...
                    <tbody id="managerOrdersTableBody">
                        <tr><td colspan="5" class="text-center py-10 text-gray-500"><i class="fa-solid fa-spinner fa-spin mr-2"></i> Đang trích xuất dữ liệu...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
...
function getManagerRevenueContent() {
    return `
        <div class="max-w-6xl mx-auto">
            <h1 class="text-2xl md:text-3xl font-extrabold text-gray-900 mb-8">Báo cáo doanh thu trực tuyến</h1>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-center">
                    <div class="text-sm font-bold text-gray-500 mb-1">Tổng Doanh Thu Hoàn Thành</div>
                    <div class="text-3xl font-black text-[#E33539]" id="revTotalAmount">0 đ</div>
                </div>
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-center">
                    <div class="text-sm font-bold text-gray-500 mb-1">Số Đơn Hàng Thành Công</div>
                    <div class="text-3xl font-black text-green-600" id="revTotalOrders">0</div>
                </div>
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-center">
                    <div class="text-sm font-bold text-gray-500 mb-1">Đơn Hàng Đang Xử Lý</div>
                    <div class="text-3xl font-black text-blue-600" id="revPendingOrders">0</div>
                </div>
            </div>

            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-6 border-b border-gray-100 font-bold text-lg text-gray-800 flex items-center gap-2">
                    <i class="fa-solid fa-clock-rotate-left text-gray-400"></i> Lịch sử giao dịch hoàn tất
                </div>
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50 text-[11px] text-gray-500 font-bold uppercase tracking-wider border-b border-gray-100">
                    ...
                    <tbody id="managerRevenueTableBody">
                        <tr><td colspan="4" class="text-center py-10 text-gray-500">Đang quét sổ sách CSDL...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
...
export function renderManagerTables(tables = []) { return getManagerLayout(getTableManagementContent(tables), 'tables'); }
...
export function initManagerEvents() {
    ...
    // ==============================================
    // LOGIC ĐỔ DỮ LIỆU VÀO TRANG ĐƠN HÀNG VÀ DOANH THU
    // ==============================================
    if (window.location.hash === '#/manager/orders') {
        fetch(`${API_BASE}/api/warehouse/orders`)
            .then(res => res.json())
            .then(orders => {
                const tbody = document.getElementById('managerOrdersTableBody');
                if (!tbody) return;
                if (!orders || orders.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="5" class="text-center py-10 text-gray-400 font-medium">Hệ thống chưa ghi nhận đơn hàng nào.</td></tr>';
                    return;
                }
                ...
                tbody.innerHTML = orders.map(o => {
                    ...
                    if(o.status === 'Chờ xử lý') statusClass = 'bg-red-50 text-red-600 border-red-100';
                    if(o.status === 'Đang chế biến') statusClass = 'bg-blue-50 text-blue-600 border-blue-100';
                    if(o.status === 'Hoàn thành') statusClass = 'bg-green-50 text-green-600 border-green-100';

                    return `
                        <tr class="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                            <td class="py-4 px-6 font-bold text-gray-900 text-xs">${o.id}</td>
                            <td class="py-4 px-6 font-bold text-gray-700">${tableName}</td>
                            <td class="py-4 px-6 text-gray-600 text-sm">${o.serverName || o.customerName || 'Khách vãng lai'}</td>
                            <td class="py-4 px-6 font-extrabold text-[#E33539]">${(o.totalAmount || 0).toLocaleString('vi-VN')} đ</td>
                            <td class="py-4 px-6"><span class="px-3 py-1 rounded border text-[11px] font-bold ${statusClass}">${o.status}</span></td>
                        </tr>
                    `;
                }).join('');
            })
    ...
    if (window.location.hash === '#/manager/revenue') {
        fetch(`${API_BASE}/api/warehouse/orders`)
            .then(res => res.json())
            .then(orders => {
                const safeOrders = orders || [];
                // Bóc tách đơn hoàn thành (ra doanh thu) và đơn đang chờ
                const completedOrders = safeOrders.filter(o => o.status === 'Hoàn thành');
                const pendingOrders = safeOrders.filter(o => o.status !== 'Hoàn thành');

                // Tính tổng doanh thu
                const totalRev = completedOrders.reduce((sum, o) => sum + (o.totalAmount || 0), 0);

                const elTotal = document.getElementById('revTotalAmount');
                const elCount = document.getElementById('revTotalOrders');
                const elPending = document.getElementById('revPendingOrders');
                const tbody = document.getElementById('managerRevenueTableBody');

                if (elTotal) elTotal.textContent = totalRev.toLocaleString('vi-VN') + ' đ';
                if (elCount) elCount.textContent = completedOrders.length;
                if (elPending) elPending.textContent = pendingOrders.length;

                if (!tbody) return;
                if (completedOrders.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="4" class="text-center py-10 text-gray-400 font-medium">Chưa có giao dịch hoàn thành nào để lập báo cáo.</td></tr>';
                    return;
                }
    ...
    // ==============================================
    // CÁC LOGIC CHUNG CỦA MODAL, MENU VÀ BÀN GIỮ NGUYÊN BÊN DƯỚI
    // ==============================================
    ...
    if (window.location.hash.includes('menu')) {
        fetch(`${API_BASE}/api/warehouse/materials`)
            .then(res => res.json())
            .then(data => { window.warehouseMaterials = data; })
            .catch(err => console.error(err));
    }
    ...
    if (realProductForm) {
        ...
        oldFormClone.addEventListener('submit', async (e) => {
            ...
            recipeRows.forEach(row => {
                ...
                if (matId && qty > 0) { productMaterials.push({ material: { id: matId }, quantity: qty }); }
            });

            const productPayload = {
                name: productName, price: productPrice, description: productDescription,
                category: { id: productCategory }, imageUrl: productImageUrl,
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (btnDeleteProduct) {
            ...
            if (confirm('Xóa món này khỏi hệ thống?')) {
                try {
